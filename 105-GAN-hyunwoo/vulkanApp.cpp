...
: impl(new Impl(createVkInstance())) 
{
    ...
    else
    {
        while(true)
        {
            ...
            int scanResult = scanf("%d", &selected);

            // If scanf fails (EOF or error), use default device 0
            if (scanResult != 1)
            {
                fprintf(stderr, "[Info] No input available, using default device 0.\n");
                selected = 0;
            }
            ...
            if (selected < 0 || selected >= (int)impl->physicalDevices.size())
            {
                ...
                // If input failed and default is invalid, break to avoid infinite loop
                if (scanResult != 1)
                {
                    fprintf(stderr, "[Error] Cannot read input and no valid default device. Exiting.\n");
                    exit(1);
                }
                continue;
            }
            ...
            // If input failed, don't loop infinitely
            if (scanResult != 1)
            {
                fprintf(stderr, "[Error] Cannot initialize device and cannot read input. Exiting.\n");
                exit(1);
            }
        }
    }
